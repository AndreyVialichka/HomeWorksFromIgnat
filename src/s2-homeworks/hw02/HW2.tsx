...
export const filterAffairs = (affairs: Array<AffairType>, filter: string): Array<AffairType> => { // need to fix any
    let fiteredAffairs = affairs.filter(affair => affair.priority === filter )
    if(filter === 'all') return affairs
    return fiteredAffairs // need to fix
}
export const deleteAffair = (affairs:  Array<AffairType>, _id: number):  Array<AffairType> => { // need to fix any

    let fiteredAffairs = affairs.filter(affair => affair._id !== _id )
...
function HW2() {
    ...
    const deleteAffairCallback = (_id: number) => { // need to fix any
        let newAffairsState = deleteAffair(affairs,_id)
        setAffairs(newAffairsState)
    }
